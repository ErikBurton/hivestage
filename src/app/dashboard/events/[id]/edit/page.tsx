...
export default function EditEventPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Admins can edit any event, others only their own
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white p-8">
      <div className="max-w-2xl mx-auto">
        <a
          href="/dashboard"
          className="text-gray-500 text-sm hover:text-yellow-400 mb-6 inline-block"
        >
          ← Back to dashboard
        </a>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`w-12 h-6 rounded-full transition-colors ${
                isFree ? "bg-yellow-400" : "bg-gray-700"
              }`}
            >
              <div
                className={`w-5 h-5 bg-white rounded-full mx-auto transition-transform ${
                  isFree ? "translate-x-3" : "-translate-x-3"
                }`}
              />
            </button>
